...
$$P_{\text{smooth}} \nrightarrow_{M_{\text{NS}}} \text{可证} \quad (\alpha \gtrsim 10^{10}, \text{Re}=10^4)$$
...
4. **等价性标注的操作性语义**：什么条件下一个等价性被归类为 $\simeq_M$ 而不是 $\equiv_M$？这个判定本身在 L3 完成。具体规则等 MCP 接入后遇到第一个案例时再定义。（参见 `note-equivalence-annotation.md`。）
...
*这份文档是工作文档，不是最终设计。符号体系会随实验迭代而演化。当前版本的目标是：让一个没参与对话的人，读完后能用这些符号标注一个已知的不可能性证明。*
