...
**目标**：在动手写代码之前，先做一次反向验证实验。

**任务**：
1. 选一个尚未显式检查的成功下界证明：**Razborov-Smolensky 的 AC⁰[p] 下界**（模 p 计数）
2. 代入 Definition 6.4，检查其判别性质（模 p 计数性质）是否在 AC⁰[p] 内部可判定
3. 如果安全：框架的归纳基础更厚，继续 Phase 1
4. 如果不安全但证明成功：框架需要修正，先修正再实现

**为什么先做这个**：原型的理论地基必须先稳固。一周的阅读比三个月的错误方向更值钱。

**输出**：`illusion/phase0-verification.md`

---

### Phase 1：两层玩具系统（本周 — 两周内）
...
**输出**：
- `illusion/src/l1_circuit.py`（或其他语言）
- `illusion/src/l2_search.py`
- `illusion/phase1-results.md`
