...
### 6.5.4 Constructive Verification

The predictive power demonstrated above is retrospective: the framework correctly diagnoses known results. A stronger test is *constructive*: if a search system is designed with self-referential safety as a hard constraint, can it independently rediscover known proof techniques?

Appendix A reports such a test. A three-layer search architecture — object-level simulation (L1), discriminating property search (L2), and self-referential safety monitor (L3) — was implemented and run on two domains: AC⁰ circuits with PARITY as target, and monotone circuits with $k$-CLIQUE as target. In both cases, the system identified the correct discriminating property (random restriction for AC⁰; subgraph projection for monotone circuits) while correctly rejecting false positives that passed the collapse metric but failed the self-referential safety check. The same architecture, with only the object-level simulator and transform library replaced, produced domain-appropriate results in both settings. Details, parameters, and reproducibility information are in Appendix A.
...
**The case studies are retrospective.** All three impossibility results analyzed in this paper were already known. The framework's predictive power — its ability to identify which proof strategies will fail before they are attempted — is supported by the barrier analysis (§6.5) and the falsifiable predictions (§6.5.2), but has not yet been tested against a genuinely novel proof attempt. The constructive verification (§6.5.4, Appendix A) strengthens the evidence by showing that the framework's conditions are *generative* — a system designed around them can rediscover known proofs — but the search space in that verification was hand-designed, and the system has not yet discovered a proof technique unknown to its designers.
