"""
L2 search loop for Frege proof complexity.

Parallel to phase5/l2_search_resolution.py.
Searches for transforms that degrade bounded-depth Frege's distinguishing advantage.
"""
...
def search(
    ...
    baseline_adv = measure_distinguishing_advantage_batch(
        d_plus, d_minus, depth_limit, n_trials, seed=seed
    )
    baseline_collapse = 1.0 - baseline_adv

    if verbose:
        print(f"Baseline: advantage={baseline_adv:.3f}, collapse={baseline_collapse:.3f}")
        print(f"Depth limit: {depth_limit}, formulas: {n_formulas}, trials: {n_trials}\n")
    ...
    for transform in FREGE_TRANSFORM_REGISTRY:
        ...
        adv_after = measure_distinguishing_advantage_batch(
            transformed_plus, transformed_minus, depth_limit, n_trials,
            seed=seed + 1000
        )
...
if __name__ == "__main__":
    results = search(depth_limit=5, n_formulas=8, n_trials=5, seed=42, verbose=True)
    print("\n--- Summary ---")
